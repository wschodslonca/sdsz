...
def main(start=1986, end=2020):
    ...
    for i in range(start, end + 1, 4):
        ...
        if i >= 1986 and i <= 2020 and i not in skip:
            t1 = threading.Thread(target=sim, args=(i,))
            t1.start()
        if i >= 1986 and i <= 2020 and i + 1 not in skip:
            t2 = threading.Thread(target=sim, args=(i + 1,))
            t2.start()
        if i >= 1986 and i <= 2020 and i + 2 not in skip:
            t3 = threading.Thread(target=sim, args=(i + 2,))
            t3.start()
        if i >= 1986 and i <= 2020 and i + 3 not in skip:
            t4 = threading.Thread(target=sim, args=(i + 3,))
            t4.start()
        if t1 is not None:
            t1.join()
        if t2 is not None:
